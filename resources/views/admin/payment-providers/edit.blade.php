...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow">
    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Edit Payment Provider</h2>
    </div>
    <div class="p-6">
        <form method="POST" action="{{ route('admin.payment-providers.update', $paymentProvider) }}">
            @csrf
            @method('PATCH')
            ...
            <div class="mb-6">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $paymentProvider->name) }}" 
            ...
            <div class="mb-6">
                ...
                <select name="provider_type" id="provider_type" 
                    ...
                    <option value="shopify" {{ old('provider_type', $paymentProvider->provider_type) == 'shopify' ? 'selected' : '' }}>Shopify</option>
                    <option value="stripe" {{ old('provider_type', $paymentProvider->provider_type) == 'stripe' ? 'selected' : '' }}>Stripe</option>
                    <option value="paypal" {{ old('provider_type', $paymentProvider->provider_type) == 'paypal' ? 'selected' : '' }}>PayPal</option>
                    <option value="other" {{ old('provider_type', $paymentProvider->provider_type) == 'other' ? 'selected' : '' }}>Other</option>
                </select>
                @error('provider_type')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <textarea name="config" id="config" rows="8" 
                          ...
                          class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 dark:bg-gray-700 dark:text-white font-mono text-sm">{{ old('config', $paymentProvider->config ? json_encode($paymentProvider->config, JSON_PRETTY_PRINT) : '') }}</textarea>
            ...
            <div class="mb-6">
                <label class="flex items-center">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $paymentProvider->is_active) ? 'checked' : '' }}
